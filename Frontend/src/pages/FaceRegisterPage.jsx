...
export default function FaceRegisterPage() {
  ...
  const refreshMeta = async () => {
    try {
      ...
      if (Array.isArray(recent)) {
        setRecentAttendance(recent.slice(-20).reverse());
      }
    } catch (e) {
  ...
  useEffect(() => {
    const fetchRealtime = async () => {
      try {
        ...
        console.error("Register page IoT poll:", e);
      }
    };
    fetchRealtime();
  ...
  useEffect(() => {
    refreshMeta();
    const id = window.setInterval(refreshMeta, 5000);
    return () => clearInterval(id);
  }, []);
